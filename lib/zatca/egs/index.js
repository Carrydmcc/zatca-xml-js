...
class EGS {
    egs_info;
    api;
    constructor(egs_info, env = api_1.ENV.SANDBOX) {
    ...
    async checkInvoiceCompliance(signed_invoice_string, invoice_hash) {
        ...
        return await this.api.compliance(this.egs_info.compliance_certificate, this.egs_info.compliance_api_secret).checkInvoiceCompliance(signed_invoice_string, invoice_hash, this.egs_info.current_invoice_uuid);
    }
    /**
    ...
    async reportInvoice(signed_invoice_string, invoice_hash) {
        ...
        return await this.api.production(this.egs_info.production_certificate, this.egs_info.production_api_secret).reportInvoice(signed_invoice_string, invoice_hash, this.egs_info.current_invoice_uuid);
    }
    /**
     ...
     * @returns Promise reporting data on success, throws error on fail.
     */
    async clearInvoice(signed_invoice_string, invoice_hash) {
        if (!this.egs_info.production_certificate || !this.egs_info.production_api_secret)
            throw new Error("EGS is missing a certificate/private key/api secret to report the invoice.");
        return await this.api.production(this.egs_info.production_certificate, this.egs_info.production_api_secret).clearInvoice(signed_invoice_string, invoice_hash, this.egs_info.current_invoice_uuid);
    }
    /**
